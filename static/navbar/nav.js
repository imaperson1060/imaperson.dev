(async function () {
    ...
    if (localStorage.getItem("auth")) {
        ...
        const response = await fetch(`/login/${auth.username}/${auth.password}/`);
...
async function login() {
    ...
    if (responseJSON.success) {
        ...
        closeLoginX.addEventListener("click", function() {
            location.reload();
        });
...
async function signup() {
    window.open("/signup.html", "signupWindow", "width=500,height=500");
};

async function forgot() {
    window.open("/forgot.html", "signupWindow", "width=500,height=350");
}
